        ...
        <div className="bg-surface px-16 py-10">
            ...
            {members.length === 0 ? (
                <div className="bg-surface-light border border-surface-lighter rounded-xl flex flex-col items-center justify-center py-20 px-6">
                    <AvatarGroup />
                    <h2 className="text-[15px] font-bold text-brand tracking-tight mb-1.5">No Subusers</h2>
                    <p className="text-[12px] font-bold text-brand/35 text-center max-w-[280px] leading-relaxed">
                ...
                <div className="bg-surface-light border border-surface-lighter rounded-xl px-[2px] pb-[2px] pt-0">
                    <div className="w-full">
                        <div className="grid grid-cols-[2fr_1fr_0.5fr] px-6 py-3">
                            <span className="text-[10px] font-bold text-brand/60 uppercase tracking-[0.2em]">User</span>
                            <span className="text-[10px] font-bold text-brand/60 uppercase tracking-[0.2em]">Permissions</span>
                            ...
                            {members.map((member, idx) => (
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        />
                                        <span className="text-[12px] font-bold text-brand">{member.email}</span>
                                    </div>
                                    <div className="flex items-center gap-1.5 flex-wrap">
                                        {(member.permissions || []).slice(0, 2).map(p => (
                                            <span key={p} className="px-2 py-0.5 rounded bg-brand/5 border border-surface-lighter text-[9px] font-bold text-brand/40 uppercase tracking-widest">
                                                {p.replace("control.", "").replace("file.", "")}
                                            </span>
                                        ))}
                                        {(member.permissions || []).length > 2 && (
                                            <span className="text-[9px] font-bold text-brand/25 uppercase tracking-widest">
            ...
            <CenterModal isOpen={inviteOpen} onClose={() => !inviting && setInviteOpen(false)} maxWidth="max-w-sm">
                <div className="p-6">
                    <div className="mb-5">
                        <h2 className="text-[16px] font-bold text-brand tracking-tight">Invite User</h2>
                        <p className="text-[11px] font-bold text-brand/30 uppercase tracking-widest mt-0.5">Grant access to this instance</p>
                    </div>
                    <label className="block text-[10px] font-bold text-brand/40 uppercase tracking-widest mb-1.5">Email address</label>
                    <input
                        type="email"
                        value={inviteEmail}
