...
export const clientApi = new Elysia({ name: "client-api" })
  ...
  .get("/servers/:id/network/allocations", async ({ request, set, params }) => {
    const limited = checkRateLimit({ request, set });
    if (limited) return limited;
    ...
    try {
      const outData = await getServerAllocations({ userId: res.body.user.id, serverId: id });
  ...
  .post("/servers/:id/power", async ({ request, set, params }) => {
    ...
    try {
      await setServerPowerState({ userId: res.body.user.id, serverId: id, state });
      const out = ok({ state }, 200);
      set.status = out.status;
      return out.body;
    } catch (err) {
      set.status = err?.status || 500;
